...
import { getAttackPaths, getRisk, getRiskTrend, getScans } from '../lib/api';
import type { Asset, AttackPath, Finding, Project, Scan } from '../types';
...
export const DashboardPage: React.FC = () => {
  ...
  const { selectedProject, assets, findings } = useOutletContext<DashboardContext>();
  ...
  useEffect(() => {
    setScanError(false);
    setAttackPathError(false);
    Promise.all([getScans(selectedProject.id), getAttackPaths(selectedProject.id), getRisk(selectedProject.id), getRiskTrend(selectedProject.id)])
      .then(([scanItems, pathItems, risk, trend]) => {
        setScans(scanItems);
  ...
  return (
    <div className="p-6 space-y-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-white">Security Dashboard</h1>
            <p className="text-sm text-slate-400 mt-0.5">{selectedProject.name} · Current backend data</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4">
        ...
        <StatCard title="Total Assets" value={assets.length} subtitle={`${activeAssets.length} active`}
          ...
          <Card title="Risk Score Trend" subtitle="30-day rolling window">
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={riskTrend} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="score" stroke="#ef4444" strokeWidth={2} fill="url(#riskGrad)" dot={{ r: 3, fill: '#ef4444' }} name="Risk Score" />
              </AreaChart>
            </ResponsiveContainer>
          </Card>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <div className="space-y-4">
          ...
          {/* Top attack paths */}
          <Card title="Attack Paths" subtitle={`${attackPaths.length} identified`} action={
            <button onClick={() => navigate('/attack-paths')} className="btn-ghost text-xs">
              <Eye className="w-3.5 h-3.5" /> View
            </button>
          }>
            <div className="space-y-2">
              {attackPathError && <p className="text-xs text-red-400">Unable to load attack-path data from the backend.</p>}
              {!attackPathError && attackPaths.length === 0 && <p className="text-xs text-slate-500">No attack paths identified for this project.</p>}
              {attackPaths.map(ap => (
                <div
                  key={ap.id}
